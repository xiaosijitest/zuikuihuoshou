from zuikuihuoshou.wx.field_view.format import format_addr_hex, format_addr_dec, format_size, format_data, format_name, format_desc
...
class field_view_imp_t:
    ...
    def on_item_selected(self):
        name = self.view.get_selected('name')
    ...
    def OnGetItemTextImp(self, item, col):
        if self.has_parent:
            if item == 0:
                if col == self.view.get_col_index('name'):
                    return '../'
                else:
                    return ''
            else:
