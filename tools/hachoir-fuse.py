...
# import os
import stat
from zuikuihuoshou.log import log
from zuikuihuoshou.field import MissingField
from zuikuihuoshou.tools import makePrintable
from zuikuihuoshou.stream import FileOutputStream
...
class HelloFS(Fuse):
    ...
    def __init__(self, input_filename, **kw):
        ...
        self.zuikuihuoshou = createParser(input_filename)
        if True:
            self.zuikuihuoshou = createEditor(self.zuikuihuoshou)
            self.readonly = False
        else:
            self.readonly = True
...
#            return -errno.EACCES
    ...
    def write(self, path, data, offset):
        if path == "/.command/writeInto":
            ...
            self.zuikuihuoshou.writeInto(stream)
        return len(data)
